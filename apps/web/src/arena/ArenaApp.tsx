...
export function ArenaApp() {
  ...
  return <main className={`arena-app ${room?.phase === "drafting" ? "is-drafting" : ""}`}>
    {!room && surface === "home" && <div className="home-scene">
      ...
      <footer className="home-footer">Draft here. Play in Clash Royale.<small>This material is unofficial and is not endorsed by Supercell.</small></footer>
    </div>}
    ...
    {room?.phase === "complete" && !finishingDraft && cards.length > 0 && <div className="finish-scene">
      ...
      <div className="deck-detail"><span>Average elixir <b>{room.export?.averageElixir?.toFixed(1) ?? "—"}</b>{room.export?.averageElixir === null ? " (Mirror varies)" : ""}<i>●</i></span><span>8 / 8 cards</span></div>
