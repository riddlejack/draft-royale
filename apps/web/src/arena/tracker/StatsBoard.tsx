...
export function StatsBoard({ credential, onSignIn }: StatsBoardProps) {
  ...
  return <section className="tracker-shell">
    ...
    <section className="tracker-filter-panel"><div className="tracker-section-heading"><div><h2><Filter size={18} /> Focus</h2><p>Every chart and game below uses the same sample.</p></div></div><div className="tracker-filter-grid">
      ...
      <label>View<select value={summary.filters.relationship} onChange={(event) => updateFilter("relationship", event.target.value as TrackerFilters["relationship"])}><option value="all">All own games</option><option value="versus">Against player</option><option value="alongside">Alongside in 2v2</option></select></label>
      ...
      <label>Mode<select value={summary.filters.mode ?? ""} onChange={(event) => updateFilter("mode", event.target.value)}><option value="">All modes</option>{summary.filterOptions.modes.map((mode) => <option key={mode.key} value={mode.key}>{displayMode(mode.name)}</option>)}</select></label>
      <label>From<input type="date" value={summary.filters.dateFrom ?? ""} onChange={(event) => updateFilter("dateFrom", event.target.value)} /></label>
      <label>Through<input type="date" value={summary.filters.dateTo ?? ""} onChange={(event) => updateFilter("dateTo", event.target.value)} /></label>
    </div></section>
    ...
    <div className="tracker-layout tracker-insights">
      ...
      <section className="tracker-panel"><div className="tracker-section-heading"><div><h2>Opposing decks</h2><p>Exact reported compositions; no invented archetypes.</p></div></div><TallyList empty="No complete opposing decks for these filters." items={summary.opponentDecks.slice(0, 6).map((item) => ({ key: item.signature, title: deckNames(item.cards), detail: "Exact opposing deck", tally: item }))} /></section>
    </div>
    <div className="tracker-layout tracker-insights">
