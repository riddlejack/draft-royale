...
export function AccountControl({ hideTrigger = false }: { hideTrigger?: boolean }) {
  ...
  async function signIn() {
    if (pending) return;
    if (newPlayer && password !== passwordConfirm) { setError("Passwords do not match."); return; }
    setPending(true); setError("");
    try {
      const response = await fetch(`/api/accounts/${newPlayer ? "register" : "login"}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(newPlayer ? { displayName: username, tag, password } : { username, password }), signal: AbortSignal.timeout(15_000),
      });
  ...
  return <>
    ...
    {open && <div className="club-account-backdrop" onClick={() => setOpen(false)}><section className="club-account-panel" role="dialog" aria-modal="true" aria-label="Player account" onClick={(event) => event.stopPropagation()}>
      <header><div><span className="club-eyebrow">DRAFT ROYALE</span><h2>{account ? account.displayName : newPlayer ? "Create a player account" : "Sign in"}</h2></div><button className="icon-button" onClick={() => setOpen(false)} aria-label="Close sign in"><X /></button></header>
      {account ? <div className="club-account-current"><p className="club-player-tag">{account.tag}</p><p>This device is signed in. Your friends, published decks, and match records stay with your player profile.</p><button className="royale-button blue" onClick={() => { clearSocialIdentity(); localStorage.removeItem("draft-royale:account"); storage.set("name", "Player"); window.location.reload(); }}>Switch player</button></div> : <form onSubmit={(event) => { event.preventDefault(); void signIn(); }}>
        ...
        {newPlayer && <><label>Clash Royale tag<input value={tag} onChange={(event) => setTag(event.target.value)} placeholder="#YOURTAG" maxLength={16} required /></label><p className="club-account-hint">A player tag is public game data; entering it does not verify account ownership.</p></>}
        <label>Password<input type="password" minLength={12} maxLength={128} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={newPlayer ? "new-password" : "current-password"} placeholder={newPlayer ? "12 characters or more" : "Password"} required /></label>
        {newPlayer && <label>Confirm password<input type="password" minLength={12} maxLength={128} value={passwordConfirm} onChange={(event) => setPasswordConfirm(event.target.value)} autoComplete="new-password" required /></label>}
        {error && <p role="alert" className="club-account-error">{error}</p>}
        <button className="royale-button gold full-width" disabled={pending}>{pending ? "Connecting…" : newPlayer ? "Create account & sign in" : "Sign in & remember device"}</button>
        <p className="club-account-hint">This device stays signed in on this site address. Do not reuse a Clash Royale or Supercell password.</p>
        <button className="text-button" type="button" onClick={() => { setNewPlayer(!newPlayer); setUsername(""); setPassword(""); setPasswordConfirm(""); setTag(""); setError(""); }}>{newPlayer ? "Back to sign in" : "Create a player account"}</button>
      </form>}
    </section></div>}
  </>;
}
