...
function DeckRow({ deck, catalog, cardsByKey, pairSelected, onTogglePair, onEdit, onCopy, onPublish, onRemove, busy }: DeckRowProps) {
  ...
  const sourceTime = timeLabel(deck.source.timestampSeconds);
  return <article className={`library-deck-row ${pairSelected ? "is-paired" : ""}`}>
    ...
    <footer className="library-deck-footer">
      ...
      <span className="library-source">{deck.source.kind === "video" ? `Verified from video${sourceTime ? ` · ${sourceTime}` : ""}` : /^https?:\/\//.test(deck.source.url ?? "") ? <a href={deck.source.url} target="_blank" rel="noreferrer">{deck.source.label}</a> : deck.source.label}</span>
...
export function DeckLibrary({ catalog, collection, onBack, onCopy, credential, onMirror }: { catalog: readonly ArenaCard[]; collection: ArenaCollection; onBack: () => void; onCopy: (value: string, message: string) => void; credential?: SocialCredential | null; onMirror?: () => void }) {
  ...
  return <section className="deck-library-screen">
    ...
    {tab === "mirror" && onMirror && <section className="pair-prompt"><Swords size={26} /><div><strong>Same deck. Settle it in the arena.</strong><span>Open a shared room, cycle through decks, and edit together.</span></div><button className="deck-new" onClick={onMirror}>Play Mirror</button></section>}
    ...
    <div className="deck-library-list">
      ...
      {!loading && !loadError && availableDecks.length === 0 && <div className="deck-library-empty"><Swords size={27} /><strong>No decks here yet.</strong><span>Make one, save it, and send it to a friend.</span><button type="button" onClick={() => setEditing(emptyDeck(createMode()))}><Plus size={15} /> Create deck</button></div>}
    </div>
  </section>;
}
